...
class SnykExportAPI:
    ...
    def __init__(self, api_token: str, org_id: Optional[str] = None, base_url: Optional[str] = None):
        ...
        self._excluded_org_ids: Set[str] = set()

    def add_excluded_org(self, org_id: str) -> None:
        """Add an organization ID to the excluded list."""
        self._excluded_org_ids.add(org_id)
    ...
    def list_organizations(self, group_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for org in orgs_data.get('data', []):
                org_id = org.get('id')
                if org_id not in self._excluded_org_ids:
                    attrs = org.get('attributes', {})
                    org_item = {
                        'id': org_id,
                        'name': attrs.get('name', 'Unnamed Organization'),
            ...
            if group_id and not results:
                ...
                for org in orgs_data.get('data', []):
                    org_id = org.get('id')
                    if org_id not in self._excluded_org_ids and org_id == group_id:
                        attrs = org.get('attributes', {})
                        results.append({
                            'id': org_id,
                            'name': attrs.get('name', 'Unnamed Organization'),
